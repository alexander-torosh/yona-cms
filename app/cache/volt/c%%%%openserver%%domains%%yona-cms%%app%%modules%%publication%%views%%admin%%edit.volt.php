<form method="post" class="ui form" action="" enctype="multipart/form-data">
    ...
    <div class="ui segment">
        ...
        <?php if ($model->getId()) { ?>
            ...
            <a href="/publication/admin/delete/<?php echo $model->getId(); ?>" class="ui button red">
                <i class="icon trash"></i> Удалить
            </a>

            <?php if ($model->getId()) { ?>
                <a class="ui blue button" target="_blank"
                   href="<?php echo $this->url->get(array('for' => 'publication', 'type' => $model->getType(), 'slug' => $model->getSlug())); ?>">
                    Посмотреть на сайте
                </a>
            <?php } ?>
    ...
    <div class="ui segment">
        <?php echo $form->renderDecorated('type'); ?>
        ...
        <?php $image = $this->helper->image(array('id' => $model->getId(), 'type' => 'publication', 'width' => 200, 'hash' => true)); ?>
